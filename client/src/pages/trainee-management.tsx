...
function TraineeManagement({ hasFullAccess }: TraineeManagementProps) {
  ...
  // Fetch phase change requests for trainers and managers
  const { data: trainerPhaseRequests = [], error: trainerPhaseRequestsError } = useQuery<PhaseRequest[]>({
    queryKey: [`/api/trainers/${user?.id}/phase-change-requests`],
    enabled: !!user?.id && (user?.role === 'trainer' || user?.role === 'team_lead'),
    staleTime: 5 * 60 * 1000, // 5 minutes cache
    refetchOnWindowFocus: false,
    onSuccess: (data) => {
      console.log("Trainer phase requests loaded:", data);
    },
    onError: (err) => {
      console.error("Error loading trainer phase requests:", err);
    }
  });
  
  // For managers, fetch requests that need their approval
  const { data: managerPhaseRequests = [], error: managerPhaseRequestsError } = useQuery<PhaseRequest[]>({
    queryKey: [`/api/managers/${user?.id}/phase-change-requests`],
    enabled: !!user?.id && (user?.role === 'manager' || user?.role === 'team_lead' || user?.role === 'admin' || user?.role === 'owner'),
    staleTime: 5 * 60 * 1000, // 5 minutes cache
    refetchOnWindowFocus: false,
    onSuccess: (data) => {
      console.log("Manager phase requests loaded:", data);
    },
    onError: (err) => {
      console.error("Error loading manager phase requests:", err);
    }
  });
  ...
  // Type definition for phase change requests
  type PhaseRequest = {
    id: number;
    batchId: number;
    batchName?: string; // Make this optional since it might come from a join
    trainerId: number;
    managerId: number | null;
    currentPhase: string;
    requestedPhase: string;
    justification: string; // Changed from reason to match the database field
    ...
    <div className="p-8 space-y-6">
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab} className="space-y-6">
        ...
        <TabsContent value="notifications">
          {/* Phase Change Requests Section */}
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="text-lg flex items-center">
                <RefreshCcw className="mr-2 h-5 w-5" /> Phase Change Requests
              </CardTitle>
              <CardDescription>
                View and manage phase change requests for your batches
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* For trainers - show their submitted requests */}
              {(user?.role === 'trainer' || user?.role === 'team_lead') && (
                <div className="space-y-4">
                  <h3 className="text-md font-medium">Your Submitted Requests</h3>
                  {trainerPhaseRequests && trainerPhaseRequests.length > 0 ? (
                    <div className="rounded-md border">
                      <Table>
                        ...
                        <TableBody>
                          {(trainerPhaseRequests as PhaseRequest[]).map((request) => (
                            <TableRow key={request.id}>
                              <TableCell className="font-medium">{request.batchName}</TableCell>
                              ...
                              <TableCell>{formatToIST(request.createdAt)}</TableCell>
                              <TableCell>
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      if (confirm('Are you sure you want to cancel this request?')) {
                                        fetch(`/api/phase-change-requests/${request.id}`, {
                                          method: 'DELETE',
                                          credentials: 'include'
                                        })
                                        .then(response => {
                                          if (response.ok) {
                                            ...
                                            toast({
                                              ...
                                              description: "The phase change request has been cancelled."
                                            });
                                          } else {
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() => {
                                    window.location.href = `/batch-details/${request.batchId}`;
                                  }}
                                >
                                  View Batch
                                </Button>
                    ...
                    <div className="py-4 text-center text-muted-foreground">
                      You haven't submitted any phase change requests yet.
                    </div>
                  )}
                </div>
              )}

              {/* For managers - show requests that need their approval */}
              {(user?.role === 'manager' || user?.role === 'team_lead' || user?.role === 'admin' || user?.role === 'owner') && (
                <div className="space-y-4 mt-6">
                  <h3 className="text-md font-medium">Pending Approval</h3>
                  {managerPhaseRequests && managerPhaseRequests.length > 0 ? (
                    <div className="rounded-md border">
                      <Table>
                        <TableHeader>
                          <TableRow>
                        ...
                        <TableBody>
                          {(managerPhaseRequests as PhaseRequest[])
                            .filter((req) => req.status === 'pending')
                            .map((request) => (
                              <TableRow key={request.id}>
                                <TableCell className="font-medium">{request.batchName}</TableCell>
                                <TableCell>{request.trainer ? request.trainer.fullName : 'Unknown'}</TableCell>
                                <TableCell>{request.currentPhase}</TableCell>
                                <TableCell>{request.requestedPhase}</TableCell>
                                <TableCell>{formatToIST(request.createdAt)}</TableCell>
                                <TableCell>
                                    ...
                                    <Button
                                      ...
                                      onClick={() => {
                                        fetch(`/api/phase-change-requests/${request.id}`, {
                                          ...
                                          body: JSON.stringify({ 
                                            status: 'approved',
                                            managerComments: 'Approved from trainee management'
                                          })
                                        })
                                        .then(response => {
                                          if (response.ok) {
                                            ...
                                            toast({
                                              ...
                                              description: "The phase change request has been approved."
                                            });
                                          } else {
                                          ...
                                          .then(response => {
                                            if (response.ok) {
                                              ...
                                              toast({
                                                ...
                                                description: "The phase change request has been rejected."
                                              });
                                            } else {
                    ...
                    <div className="py-4 text-center text-muted-foreground">
                      No pending phase change requests.
                    </div>
                  )}
                </div>
              )}
          ...
          <Alert>
            <AlertDescription>
              Other notifications about batch progress and important updates will appear here.
            </AlertDescription>
          </Alert>
        </TabsContent>
      </Tabs>
