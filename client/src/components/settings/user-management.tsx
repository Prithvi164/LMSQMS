...
export function UserManagement() {
  ...
  const { data: allUsers = [] } = useQuery<User[]>({ // Fetch all users for manager/location resolution
    queryKey: ["/api/users"],
    enabled: !!user,
  });
    ...
    reader.onload = async (e) => {
      try {
        ...
        jsonData.forEach((row, index) => {
          ...
          // Role validation - match manual creation roles
          if (!row.Role || !['admin', 'manager', 'team_lead', 'qualityassurance', 'trainer', 'advisor'].includes(row.Role.toLowerCase())) {
            errors.push(`Row ${index + 1}: Invalid role. Must be one of: admin, manager, team_lead, qualityassurance, trainer, advisor`);
          }

          // Manager validation
          if (row.Manager) {
            const managerExists = allUsers.some(u =>
              u.username.toLowerCase() === row.Manager.toLowerCase() ||
              (u.fullName && u.fullName.toLowerCase() === row.Manager.toLowerCase())
            );
            if (!managerExists) {
              errors.push(`Row ${index + 1}: Manager '${row.Manager}' not found in system`);
            }
          }

          // Location validation
          if (row.Location) {
            const locationExists = orgSettings?.locations?.some(l =>
              l.name.toLowerCase() === row.Location.toLowerCase()
            );
            if (!locationExists) {
              errors.push(`Row ${index + 1}: Location '${row.Location}' not found in system`);
            }
          }
          ...
          if (row.Username && !/^[a-zA-Z0-9._-]+$/.test(row.Username)) {
            errors.push(`Row ${index + 1}: Username can only contain letters, numbers, dots, underscores, and hyphens`);
          }

          // Date validations
          if (row["Date of Joining"] && !isValidDate(row["Date of Joining"])) {
            errors.push(`Row ${index + 1}: Invalid Date of Joining format. Use YYYY-MM-DD`);
          }
          if (row["Date of Birth"] && !isValidDate(row["Date of Birth"])) {
            errors.push(`Row ${index + 1}: Invalid Date of Birth format. Use YYYY-MM-DD`);
          }
        });
  ...
  const importUsersMutation = useMutation({
    mutationFn: async (users: ImportedUser[]) => {
      ...
      const transformedUsers = users.map(user => {
        // Find manager ID from username or full name
        const manager = allUsers.find(u =>
          u.username.toLowerCase() === user.Manager.toLowerCase() ||
          (u.fullName && u.fullName.toLowerCase() === user.Manager.toLowerCase())
        );

        // Find location ID from name
        const location = orgSettings?.locations?.find(l =>
          l.name.toLowerCase() === user.Location.toLowerCase()
        );

        // Generate a secure temporary password - same logic as manual creation
        const tempPassword = `${user.Username}@${new Date().getFullYear()}`;
        ...
        return {
          ...
          active: true, // Default to active as in manual creation
          certified: false, // Default to uncertified as in manual creation
    ...
    onSuccess: () => {
      toast({
        title: "Success",
        description: "Users imported successfully. Temporary passwords have been set as 'username@currentYear'.",
      });
      queryClient.invalidateQueries({ queryKey: ["/api/users"] });
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <div className="flex flex-col gap-4">
                  ...
                  <TableRow key={u.id}>
                    ...
                    <TableCell>
                      <Switch
                        ...
                        onCheckedChange={() => toggleUserStatus(u.id, u.active, u.role)}
                      />
                    </TableCell>
            ...
            {/* Import Preview and Delete Confirmation Dialogs */}
            {ImportPreviewDialog()}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[150px]">Last Working Day</TableHead>
                  <TableHead className="w-[100px]">Status</TableHead>
                  <TableHead className="w-[100px] text-right">Actions</TableHead>                </TableRow>
              </TableHeader>
              <TableBody>
                {currentUsers.map((u) => (
