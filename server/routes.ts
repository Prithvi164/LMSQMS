...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Add quiz template update route
  app.patch("/api/quiz-templates/:id", async (req, res) => {
    if (!req.user || !req.user.organizationId) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    ...
    try {
      ...
      console.log('Updating quiz template:', templateId, 'with data:', req.body);
  ...
  // Add quiz attempt creation route
  app.post("/api/quiz-attempts", async (req, res) => {
    ...
    try {
      ...
      console.log('Creating quiz attempt for template:', templateId);
      ...
      if (template.organizationId !== req.user.organizationId) {
        return res.status(403).json({ message: "Unauthorized to use this template" });
      }

      // Get random questions based on template settings
      ...
      const attempt = await storage.createQuizAttempt({
        ...
        startTime: new Date(),
        endTime: new Date(Date.now() + template.timeLimit * 60000), // Convert minutes to milliseconds
        status: 'in_progress'
      });
      ...
      res.status(400).json({ 
        message: error.message || "Failed to create quiz attempt",
        details: error.stack
      });
    }
  });

  // Add quiz attempt detail route
  app.get("/api/quiz-attempts/:id", async (req, res) => {
    ...
    try {
      ...
      // Check if attempt belongs to user's organization
      if (attempt.organizationId !== req.user.organizationId) {
        return res.status(403).json({ message: "Unauthorized to view this attempt" });
      }

      console.log('Retrieved quiz attempt:', attempt);
      res.json(attempt);
    } catch (error: any) {
  ...
  // Add route to get questions for an attempt
  app.get("/api/quiz-attempts/:id/questions", async (req, res) => {
    if (!req.user || !req.user.organizationId) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    ...
    try {
      const attemptId = parseInt(req.params.id);
      console.log('Fetching questions for quiz attempt:', attemptId);
      ...
      // Check if attempt belongs to user's organization
      if (attempt.organizationId !== req.user.organizationId) {
        return res.status(403).json({ message: "Unauthorized to view these questions" });
      }

      // Get all questions for this attempt
      const questions = await Promise.all(
        attempt.questions.map(async (questionId) => {
      ...
      console.log(`Retrieved ${sanitizedQuestions.length} questions for attempt`);
  ...
  // Add route to submit quiz answers
  app.post("/api/quiz-attempts/:id/submit", async (req, res) => {
    ...
    try {
      ...
      console.log('Submitting answers for quiz attempt:', attemptId);
      ...
      console.log('Successfully submitted quiz attempt:', {
