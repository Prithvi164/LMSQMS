...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Add trainee management endpoints after the batch management endpoints
  app.get("/api/organizations/:orgId/batches/:batchId/trainees", async (req, res) => {
    ...
    try {
      ...
      // Check if user belongs to the organization
      if (req.user.organizationId !== orgId) {
        return res.status(403).json({ message: "You can only view trainees in your own organization's batches" });
      }
  ...
  // Get available trainees that can be added to a batch
  app.get("/api/organizations/:orgId/available-trainees", async (req, res) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    ...
    try {
      ...
      // Get all users with trainee category in the organization
      const trainees = await storage.listUsers(orgId, { category: 'trainee' });
      res.json(trainees);
    } catch (error: any) {
  ...
  app.post("/api/organizations/:orgId/batches/:batchId/trainees", async (req, res) => {
    ...
    try {
      ...
      // Check if user belongs to the organization
      if (req.user.organizationId !== orgId) {
        return res.status(403).json({ message: "You can only add trainees to your own organization's batches" });
      }

      const result = await storage.addTraineeToBatch({
        batchId,
        traineeId,
        status: 'active'
      });
  ...
  app.delete("/api/organizations/:orgId/batches/:batchId/trainees/:traineeId", async (req, res) => {
    ...
    try {
      ...
      // Check if user belongs to the organization
      if (req.user.organizationId !== orgId) {
        return res.status(403).json({ message: "You can only remove trainees from your own organization's batches" });
      }

      await storage.removeTraineeFromBatch(batchId, traineeId);
