...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Add endpoint to get trainees by batch
  app.get("/api/batches/:batchId/trainees", async (req, res) => {
    ...
    try {
      ...
      // Get batch details to verify organization access
      const batch = await storage.getBatch(batchId);
      if (!batch || batch.organizationId !== req.user.organizationId) {
        return res.status(403).json({ message: "Forbidden" });
      }

      // Get trainees enrolled in this batch
      const trainees = await db.query.users.findMany({
        where: and(
          eq(users.organizationId, req.user.organizationId),
          eq(users.role, 'trainee'),
          ...
          inArray(
            users.id,
            db.select({ id: userBatchProcesses.userId })
  ...
  // Add endpoint to get active batches
