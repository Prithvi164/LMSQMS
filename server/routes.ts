...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/evaluation-templates/:templateId/reorder-pillars", async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    ...
    try {
      const templateId = parseInt(req.params.templateId);
      const { pillarId, newIndex } = req.body;

      if (!templateId || !pillarId || typeof newIndex !== 'number') {
      ...
      await storage.updatePillarOrder(templateId, pillarId, newIndex);
  ...
  app.post("/api/evaluation-pillars/:pillarId/reorder-parameters", async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    ...
    try {
      const pillarId = parseInt(req.params.pillarId);
      const { parameterId, newIndex } = req.body;

      if (!pillarId || !parameterId || typeof newIndex !== 'number') {
        return res.status(400).json({ message: "Invalid request parameters" });
      }

      // Get the pillar to verify ownership
      const pillar = await storage.getEvaluationPillar(pillarId);
      if (!pillar) {
        return res.status(404).json({ message: "Pillar not found" });
      }
      ...
      await storage.updateParameterOrder(pillarId, parameterId, newIndex);
      res.json({ success: true });
    } catch (error: any) {
