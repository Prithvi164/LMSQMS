...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Trainee Progress Tracking Routes
  
  // Get trainee progress for a specific trainee in a batch
  app.get("/api/batches/:batchId/trainees/:traineeId/progress", async (req, res) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    ...
    try {
      ...
      // Only trainers, managers, or the trainee themselves should be able to access
      const isTrainer = await db
        .select()
        .from(organizationBatches)
        .where(
          and(
            eq(organizationBatches.id, batchId),
            eq(organizationBatches.trainerId, req.user.id)
          )
        )
        .limit(1);
      
      const isOwnProgress = traineeId === req.user.id;
      
      if (!isTrainer.length && !isOwnProgress && !['manager', 'admin', 'owner'].includes(req.user.role)) {
        return res.status(403).json({ message: "Not authorized to view this trainee's progress" });
      }

      const progress = await storage.getTraineeProgress(traineeId, batchId);
      
      if (!progress) {
        return res.status(404).json({ message: "Trainee progress not found" });
      }
  ...
  // Create or update trainee progress
  app.post("/api/batches/:batchId/trainees/:traineeId/progress", async (req, res) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    ...
    try {
      ...
      // Only trainers or managers should be able to create/update progress
      if (!['trainer', 'manager', 'admin', 'owner'].includes(req.user.role)) {
        return res.status(403).json({ message: "Not authorized to update trainee progress" });
      }
      
      // Check if progress already exists
      const existingProgress = await storage.getTraineeProgress(traineeId, batchId);
      ...
      if (existingProgress) {
        ...
        progress = await storage.updateTraineeProgress(traineeId, batchId, progressData);
      } else {
        // Create new progress
        progress = await storage.createTraineeProgress({
          ...progressData,
          createdAt: new Date()
        });
      }
      ...
      console.error("Error creating/updating trainee progress:", error);
      res.status(400).json({ message: error.message });
    }
  });
  
  // List all trainees' progress in a batch
  app.get("/api/batches/:batchId/progress", async (req, res) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    
    try {
      const batchId = parseInt(req.params.batchId);
      
      // Only trainers, managers, admins, or owners can view all progress
      if (!['trainer', 'manager', 'admin', 'owner'].includes(req.user.role)) {
        return res.status(403).json({ message: "Not authorized to view batch progress" });
      }
      
      const progressRecords = await storage.listTraineeProgressByBatch(batchId);
      res.json(progressRecords);
    } catch (error: any) {
      console.error("Error listing batch progress:", error);
      res.status(500).json({ message: error.message });
    }
  });
  ...
  app.post("/api/assessments", async (req, res) => {
    ...
    try {
      // Only trainers or managers should be able to create assessments
      if (!['trainer', 'manager', 'admin', 'owner'].includes(req.user.role)) {
      ...
      // If assessment was passed and trainee progress exists, update the trainee's phase
      if (assessment.status === 'passed' && assessment.traineeId && assessment.batchId) {
        const traineeProgress = await storage.getTraineeProgress(assessment.traineeId, assessment.batchId);
        
        if (traineeProgress) {
          let updatedPhase = traineeProgress.currentPhase;
          
          // Update phase based on passed assessment type
          if (assessment.type === 'written' && traineeProgress.currentPhase === 'training') {
            updatedPhase = 'certification';
          } else if (assessment.type === 'certification' && traineeProgress.currentPhase === 'certification') {
            updatedPhase = 'ojt';
          } else if (assessment.type === 'ojt_certification' && traineeProgress.currentPhase === 'ojt') {
            updatedPhase = 'completed';
          }
          
          if (updatedPhase !== traineeProgress.currentPhase) {
            await storage.updateTraineeProgress(assessment.traineeId, assessment.batchId, {
  ...
  app.get("/api/assessments/:id", async (req, res) => {
    ...
    try {
      ...
      const isOwnAssessment = assessment.traineeId === req.user.id;
      const isAssessor = assessment.conductedBy === req.user.id;
      const hasPermission = ['trainer', 'manager', 'admin', 'owner'].includes(req.user.role);
      
      if (!isOwnAssessment && !isAssessor && !hasPermission) {
  ...
  // Get all assessments for a trainee in a batch
  app.get("/api/batches/:batchId/trainees/:traineeId/assessments", async (req, res) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    ...
    try {
      ...
      const isOwnAssessments = traineeId === req.user.id;
      const hasPermission = ['trainer', 'manager', 'admin', 'owner'].includes(req.user.role);
      
      if (!isOwnAssessments && !hasPermission) {
        return res.status(403).json({ message: "Not authorized to view these assessments" });
      }
      
      const assessments = await storage.getAssessmentsByTrainee(traineeId, batchId, type);
  ...
  // Refresher Session Routes
  
  // Create a new refresher session
  app.post("/api/refresher-sessions", async (req, res) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    
    try {
      // Only trainers or managers should be able to create refresher sessions
      if (!['trainer', 'manager', 'admin', 'owner'].includes(req.user.role)) {
        return res.status(403).json({ message: "Not authorized to create refresher sessions" });
      }
      ...
      const session = await storage.createRefresherSession(sessionData);
  ...
  // Update a refresher session
  app.patch("/api/refresher-sessions/:id", async (req, res) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    ...
    try {
      ...
      const session = await storage.getRefresherSession(sessionId);
      
      if (!session) {
        return res.status(404).json({ message: "Refresher session not found" });
      }
      
      // Only the trainer who created the session or managers/admins can update it
      if (session.trainerId !== req.user.id && !['manager', 'admin', 'owner'].includes(req.user.role)) {
        return res.status(403).json({ message: "Not authorized to update this refresher session" });
      }
      
      const updatedSession = await storage.updateRefresherSession(sessionId, {
        ...req.body,
        updatedAt: new Date()
      });
  ...
  // Mark a refresher session as completed
  app.patch("/api/refresher-sessions/:id/complete", async (req, res) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    ...
    try {
      ...
      // Only the trainer who created the session or managers/admins can complete it
      if (session.trainerId !== req.user.id && !['manager', 'admin', 'owner'].includes(req.user.role)) {
        return res.status(403).json({ message: "Not authorized to complete this refresher session" });
      }
      
      const completedSession = await storage.completeRefresherSession(sessionId);
  ...
  // Get all refresher sessions for a trainee in a batch
  app.get("/api/batches/:batchId/trainees/:traineeId/refresher-sessions", async (req, res) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    ...
    try {
      ...
      // Check if user has permission
      const isOwnSessions = traineeId === req.user.id;
      const hasPermission = ['trainer', 'manager', 'admin', 'owner'].includes(req.user.role);
      
      if (!isOwnSessions && !hasPermission) {
        return res.status(403).json({ message: "Not authorized to view these refresher sessions" });
      }
      
      const sessions = await storage.getRefresherSessionsByTrainee(traineeId, batchId);
      res.json(sessions);
    } catch (error: any) {
