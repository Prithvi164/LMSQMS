...
import { 
  ...
  EvaluationPillar,
  traineePhaseStatus,
  traineePhaseStatusEnum,
  insertTraineePhaseStatusSchema,
  TraineePhaseStatus,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Trainee Phase Status routes
  app.post("/api/trainee-phase-status", async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    ...
    try {
      ...
      const validatedData = insertTraineePhaseStatusSchema.parse(req.body);
      ...
      const hasBatchAccess = await userHasBatchAccess(req.user.id, validatedData.batchId);
      if (!hasBatchAccess) {
        return res.status(403).json({ message: "You don't have access to this batch" });
      }
      
      // Check if a status already exists for this trainee-batch-phase
      const existingStatus = await storage.getTraineePhaseStatus(
      ...
      if (existingStatus) {
        ...
        const updatedStatus = await storage.updateTraineePhaseStatus(
          validatedData.traineeId,
          validatedData.batchId,
          validatedData.phase,
          {
            ...
            evaluatorId: validatedData.evaluatorId || req.user.id, // Default to current user if not specified
            lastEvaluationDate: validatedData.lastEvaluationDate || new Date().toISOString().split('T')[0],
          }
        );
      ...
      // Create new status
      const status = await storage.createTraineePhaseStatus(validatedData);
      ...
      res.status(500).json({ message: "Failed to create trainee phase status" });
    }
  });
  
  app.get("/api/batch/:batchId/trainee-phase-status", async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    ...
    try {
      ...
      const statuses = await storage.listTraineePhaseStatuses(batchId, phase);
      res.json(statuses);
    } catch (error) {
      console.error('Error fetching trainee phase statuses:', error);
      res.status(500).json({ message: "Failed to fetch trainee phase statuses" });
    }
  });
  
  app.get("/api/trainee/:traineeId/batch/:batchId/phase-status", async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    ...
    try {
      ...
      if (phase) {
        // Get status for specific phase
        const status = await storage.getTraineePhaseStatus(traineeId, batchId, phase);
        if (!status) {
          return res.status(404).json({ message: "Trainee phase status not found" });
        }
        return res.json(status);
      } else {
        // Get all phase statuses for this trainee in the batch
        const statuses = await storage.getTraineeStatusHistory(traineeId, batchId);
        return res.json(statuses);
      }
    } catch (error) {
      console.error('Error fetching trainee phase status:', error);
      res.status(500).json({ message: "Failed to fetch trainee phase status" });
    }
  });
  
  app.put("/api/trainee/:traineeId/batch/:batchId/phase/:phase/status", async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    ...
    try {
      ...
      if (!['induction', 'training', 'certification', 'ojt', 'ojt_certification'].includes(phase)) {
        return res.status(400).json({ message: "Invalid phase" });
      }
      
      // Check if status exists
      const existingStatus = await storage.getTraineePhaseStatus(traineeId, batchId, phase);
      if (!existingStatus) {
        return res.status(404).json({ message: "Trainee phase status not found" });
      }
      ...
      const updateData = {
        ...
        evaluatorId: req.body.evaluatorId || req.user.id,
        lastEvaluationDate: req.body.lastEvaluationDate || new Date().toISOString().split('T')[0],
      };
      ...
      res.status(500).json({ message: "Failed to update trainee phase status" });
    }
  });
