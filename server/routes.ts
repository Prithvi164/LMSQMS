...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/attendance", async (req, res) => {
    ...
    try {
      ...
      // Get updated trainee data with attendance
      const [updatedTrainee] = await db
        .select({
          id: userBatchProcesses.userId,
          status: attendance.status,
          lastUpdated: attendance.updatedAt,
          user: {
            id: users.id,
      ...
      console.log('Updated trainee record:', updatedTrainee);
  ...
  app.post("/api/attendance", async (req, res) => {
    ...
    try {
      const { traineeId, status, date, organizationId, batchId, phase, markedById } = req.body;
