...
debugLog("Starting server initialization - Before any imports");

try {
  debugLog("Starting express app creation");
  const app = express();
  debugLog("Express app created successfully");
  ...
  (async () => {
    try {
      ...
      debugLog("About to register routes...");
      ...
      app.use((err: any, _req: Request, res: Response, _next: NextFunction) => {
        ...
        debugLog(`Error middleware caught:`, err);
        log(`Error: ${message}`);
      ...
      debugLog("Fatal error during async initialization:", error);
  ...
  debugLog("Fatal error during initial setup:", error);
  log(`Failed during initial setup: ${error}`);
  process.exit(1);
}