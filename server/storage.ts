...
export interface IStorage {
  ...
  listUserLineOfBusinesses(organizationId: number, userId: number): Promise<OrganizationLineOfBusiness[]>;
  ...
  listUserProcesses(organizationId: number, userId: number): Promise<OrganizationProcess[]>;

  // Location operations
  listLocations(organizationId: number): Promise<OrganizationLocation[]>;
  // New method to filter locations by user assignment
  listUserLocations(organizationId: number, userId: number): Promise<OrganizationLocation[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async listUserLineOfBusinesses(organizationId: number, userId: number): Promise<OrganizationLineOfBusiness[]> {
    try {
      ...
      // For other users, return only assigned LOBs from userProcesses table
      const lobs = await db
        .select({
          id: organizationLineOfBusinesses.id,
          name: organizationLineOfBusinesses.name,
  ...
  async listUserLocations(organizationId: number, userId: number): Promise<OrganizationLocation[]> {
    try {
      ...
      // For other users, return only assigned locations from userProcesses table
      const locations = await db
        .select({
