...
export class DatabaseStorage implements IStorage {
  ...
  async getQuizResultsByBatch(batchId: number): Promise<{
    traineeId: number,
    fullName: string,
    employeeId: string,
    quizzes: {
    ...
    try {
      console.log(`Fetching quiz results for batch ${batchId}`);
      
      // First, get all trainees in the batch
      const trainees = await this.getBatchTrainees(batchId);
      const traineeIds = trainees.map(t => t.userId);
      
      if (traineeIds.length === 0) {
        console.log('No trainees found in the batch');
        return [];
      }
      
      // Get all quiz attempts for these trainees
      ...
      // Group the attempts by user
      const resultsByUser: Record<number, {
        traineeId: number,
        fullName: string,
        employeeId: string,
        quizzes: {
      ...
      // Get additional user info for each trainee
      const traineeInfo = await db
        .select({
          id: users.id,
          fullName: users.fullName,
          employeeId: users.employeeId
      ...
      // Create a lookup for trainee info
      const traineeInfoMap = traineeInfo.reduce((acc, trainee) => {
        acc[trainee.id] = {
          fullName: trainee.fullName,
          employeeId: trainee.employeeId
        };
        return acc;
      }, {} as Record<number, { fullName: string, employeeId: string }>);
      
      // Initialize results for each trainee
      traineeIds.forEach(traineeId => {
        const info = traineeInfoMap[traineeId] || { fullName: 'Unknown', employeeId: 'Unknown' };
        resultsByUser[traineeId] = {
          traineeId,
      ...
      // Populate quiz results for each trainee
      attempts.forEach(attempt => {
        const status: 'pass' | 'fail' = attempt.score >= attempt.passingScore ? 'pass' : 'fail';
        
        if (resultsByUser[attempt.userId]) {
          resultsByUser[attempt.userId].quizzes.push({
            quizId: attempt.quizId,
      ...
      return Object.values(resultsByUser);
    } catch (error) {
      console.error('Error fetching quiz results by batch:', error);
      throw error;
    }
  }
  
  async getQuizResultsByUser(userId: number): Promise<{
    quizId: number,
    quizName: string,
    score: number,
    passingScore: number,
    ...
    try {
      ...
      return attempts.map(attempt => ({
        ...
        score: attempt.score,
        passingScore: attempt.passingScore,
        status: attempt.score >= attempt.passingScore ? 'pass' : 'fail',
        completedAt: attempt.completedAt.toISOString()
      }));
    } catch (error) {
